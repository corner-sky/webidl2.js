...
(() => {
  function write(ast, customizer) {
    function flatten(array) {
      return [].concat(...array);
    }

    function output(texts, args) {
      const results = [];
      for (let i = 0; i < args.length; i++) {
        if (Array.isArray(args[i])) {
          results.push(texts[i], ...args[i]);
        } else {
          results.push(texts[i], args[i]);
        }
      }
      results.push(texts[texts.length - 1]);
      return results;
    }
    
    function trivia(text) {
      if (customizer && customizer.trivia) {
        return customizer.trivia(text);
      }
      return text;
    }

    function reference(name, components) {
      if (customizer && customizer.reference) {
        return customizer.reference(name, components);
      }
      return components;
    }
    ...
    function type(it) {
      ...
      if (it.union) ret.push(...output`${trivia(it.trivia.open)}(${flatten(it.idlType.map(type))}${trivia(it.trivia.close)})`);
      else if (it.generic) {
        ret.push(trivia(it.trivia.base));
        ret.push(reference(it.generic.value));
        ret.push(...output`${trivia(it.generic.trivia.open)}<${flatten(it.idlType.map(type))}${trivia(it.generic.trivia.close)}>`);
      } else if (typeof it.idlType === "string") {
        const typeName = [];
        if (it.prefix) typeName.push(trivia(it.prefix.trivia), it.prefix.value);
        typeName.push(trivia(it.trivia.base), it.baseName);
        if (it.postfix) typeName.push(trivia(it.postfix.trivia), it.postfix.value);
        customizer.reference(it.idlType, typeName);
      } else {
        ret.push(...type(it.idlType));
      }
      if (it.nullable) ret.push(...output`${trivia(it.nullable.trivia)}?`);
      if (it.separator) ret.push(trivia(it.separator.trivia), it.separator.value);
    ...
    function extended_attributes(eats) {
      if (!eats) return [];
      return output`${trivia(eats.trivia.open)}[${flatten(eats.items.map(make_ext_at))}${trivia(eats.trivia.close)}]`;
    }
    ...
    function iterate(things) {
      if (!things) return;
      if (customizer && !customizer.document) {
        throw new Error("A customizer must include a valid `document` function");
      }
      const converted = things.map(dispatch);
      if (customizer) {
        return customizer.document(converted);
      }
      else return converted.join("");
    }
    return iterate(ast);
  }
