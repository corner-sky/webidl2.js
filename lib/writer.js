  ...
  function write(ast, customizer) {
    ...
    function type(it) {
      ...
      if (it.union) ret.push(...output`${trivia(it.trivia.open)}(${flatten(it.idlType.map(type))}${trivia(it.trivia.close)})`);
        ...
        ret.push(...output`${trivia(it.trivia.base)}${it.generic.value}${it.generic.trivia.open}<${it.idlType.map(type).join("")}${it.generic.trivia.close}>`)
        ...
        if (customizer && customizer.reference) {
          ret.push(customizer.reference(it, typeName));
        } else {
          ret.push(...typeName);
        }
      } else {
