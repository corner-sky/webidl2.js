...
(() => {
  function write(ast) {
    ...
    function argument(arg) {
      ...
      const optional = arg.optional ? `${arg.optional.trivia}optional` : "";
      const idlType = type(arg.idlType);
      const variadic = arg.variadic ? `${arg.variadic.trivia}...` : "";
      const name =  `${arg.trivia.name}${arg.escapedName}`;
      const def = arg.default ? default_(arg.default) : "";
      const sep = arg.separator ? `${arg.separator.trivia}${arg.separator.value}` : "";
      return `${ext}${optional}${idlType}${variadic}${name}${def}${sep}`;
    }
    function identifier(id) {
      const value = id.trivia + id.value;
      const sep = id.separator ? id.separator.trivia + id.separator.value : "";
      return `${value}${sep}`;
    }
    function ext_rhs(rhs) {
      if (rhs.type === "identifier") {
        return `${rhs.trivia.assign}=${rhs.trivia.value}${rhs.value}`;
      }
      const assign = `${rhs.trivia.assign}=`;
      // eslint-disable-next-line no-console
      console.log(rhs.value, rhs.type);
      const ids = rhs.value.map(identifier).join("");
      const parens = `${rhs.trivia.open}(${ids}${rhs.trivia.close})`;
      return `${assign}${parens}`;
    }
    function make_ext_at(it) {
      const name = it.trivia.name + it.name;
      const rhs = it.rhs ? ext_rhs(it.rhs) : "";
      const signature = it.signature ? `${it.signature.trivia.open}(${it.signature.arguments.map(argument).join("")}${it.signature.trivia.close})` : "";
      const sep = it.separator ? `${it.separator.trivia}${it.separator.value}` : "";
      return `${name}${rhs}${signature}${sep}`;
    }
